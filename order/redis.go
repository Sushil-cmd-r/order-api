...
func (r *RedisRepo) FindByID(ctx context.Context, id string) (Order, error) {
	key := orderIDKey(id)
	...
	if err != nil {
		return Order{}, fmt.Errorf("failed to decode order: %w", err)
	}

	return order, nil
}

func (r *RedisRepo) DeleteById(ctx context.Context, id string) error {
	key := orderIDKey(id)
